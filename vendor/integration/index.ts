...
export default ({ config: _themeConfig = 'src/config.yaml' } = {}): AstroIntegration => {
  let cfg: AstroConfig;
  return {
    name: 'metaintelligence-integration',
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('metaintelligence');

        const virtualModuleId = 'metaintelligence:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-metaintelligence-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`MetaIntelligence \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`MetaIntelligence config has been loaded.`);
        }
      },
